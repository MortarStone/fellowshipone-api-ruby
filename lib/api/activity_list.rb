module FellowshipOne
  ...
  class ActivityList
    ...
    def initialize(options = {})    
      reader = options[:reader] || FellowshipOne::ActivityListReader.new(options)
    ...
    # @return Activity
    def [](index)
      Activity.new( @json_data[index] ) if @json_data[index]
    end


    # This method is needed for Enumerable.
    def each &block
      @json_data.each{ |cont_recpt| yield( Activity.new(cont_recpt) )}
    end
